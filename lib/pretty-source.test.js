...
// Remove `__dirname` from the path, otherwise tests will fail,
// if run from another machine, since paths will always be different
const cleanPath = output => output.replace(path.resolve(__dirname, '..') + '/', '')
...
const line9 = 'line 9'
`)
  t.matchSnapshot(cleanPath(pretty({
